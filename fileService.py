...
def delete_untagged_files(s3_client, account_id):
    ...
    page_iterator = paginator.paginate(
        Bucket=storage_bucket, PaginationConfig={"PageSize": 1000}
    )
    evaluated_files = 0
    deleted_files = 0
                ...
                if MetadataKey not in metadata:
                    ...
                    if len(objects_to_delete) >= 999:
                        ...
                        delete_response = s3_client.delete_objects(
                            Bucket=storage_bucket, Delete={"Objects": objects_to_delete}
                        )
...
def fetch_metadata(s3_client, account_id, key):
    storage_bucket = f"nva-resource-storage-{account_id}"
    return s3_client.head_object(Bucket=storage_bucket, Key=key)["Metadata"]
...
def update_file_metadata(s3_resource, publication_identifier, file_key, bucket_name):
    target = s3_resource.Object(bucket_name, file_key)

    if MetadataKey in target.metadata:
        return 0
    else:
        target.metadata.update({"nva-publication-identifier": publication_identifier})
...
if __name__ == "__main__":
    ...
    argParser.add_argument(
        "command", choices=["tag-files", "delete-untagged-files", "reset-tags"]
    )
    ...
    if args.command == "tag-files":
        tag_referenced_files(
            _dynamodb_client, _s3_resource, _accountId, _resources_table_name
        )
