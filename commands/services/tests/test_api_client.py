...
def _seed_aws_resources():
    ...
    ssm.put_parameter(Name="/NVA/CognitoUri", Value="https://cognito.example.org", Type="String")
    boto3.client("secretsmanager").create_secret(
        Name="BackendCognitoClientCredentials",
        SecretString=json.dumps({"backendClientId": "id", "backendClientSecret": "secret"}),
    )
...
def test_auth_header_returns_bearer_token_from_cognito():
    _seed_aws_resources()
    responses.add(responses.POST, COGNITO_TOKEN_URL, json={"access_token": "fresh", "expires_in": 3600})
...
def test_auth_header_refreshes_when_token_expired():
    _seed_aws_resources()
    responses.add(responses.POST, COGNITO_TOKEN_URL, json={"access_token": "first", "expires_in": -1})
    responses.add(responses.POST, COGNITO_TOKEN_URL, json={"access_token": "second", "expires_in": 3600})
    client = ApiClient(session=boto3.Session())
