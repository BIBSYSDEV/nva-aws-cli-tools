...
class DynamodbExport:
    ...
    def get_table(self):
        ...
        table_name = next((name for name in table_names if re.match(self.table_pattern, name)), None)
    ...
    def _inflate_batch(self, items):
        inflated_items = []
        for item in items:
            item = {k: (base64.b64encode(v.value).decode() if isinstance(v, Binary) else v) for k, v in item.items()}
...
if __name__ == '__main__':
    ...
    condition = Key('PK0').eq(f'Resource:bb3d0c0c-5065-4623-9b98-5810983c2478:ntnu@194.0.0.0')
    batch_size = 700
