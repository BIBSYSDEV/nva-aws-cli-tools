...
@click.option('-i', '--intended_purpose', required=True, help='The intended purpose. e.g. oslomet-thesis-integration')
@click.option('-s', '--scopes', required=True, help='Comma-separated list of scopes without whitespace, e.g., https://api.nva.unit.no/scopes/third-party/publication-read,https://api.nva.unit.no/scopes/third-party/publication-upsert')
def create_external(profile:str, customer:str, intended_purpose:str, scopes:str) -> None:
    external_user = ExternalUserService(profile).create(customer, intended_purpose, scopes.split(','))
